...
<div class="d-flex flex-column align-items-stretch flex-shrink-0 bg-white overflow-auto" style="padding-left:60px;width:330px;height:90%;position:fixed;z-index:4;">
    <c:forEach var="sday" begin="1" end="${days}">
        <div class="list-group list-group-flush border-bottom day-plans" data-date="${sday}">
            <div class="list-group-item py-3 lh-sm" style="background-color: #dff0d8">
                <div class="d-flex w-100 align-items-center">
                    <strong class="mb-1">DAY ${sday} 세부일정</strong>
                    <input hidden name="sDay" value="${sday}">
                </div>
            </div>
            <c:forEach var="i" begin="0" end="${schedules.size()}">
                <c:if test="${schedules[i].sday eq sday}">
                    <div class='list-group-item list-group-item-action py-3 lh-sm plans-detail' data-sday='${sday}' data-y="${schedules[i].y}" data-x="${schedules[i].x}" data-snum="${schedules[i].snum}"  onclick="planClick(${schedules[i].y}, ${schedules[i].x}, '${schedules[i].spoint}')">
                        <div class='d-flex w-100 align-items-center justify-content-between'>
                            <i class='me-2 text-muted'>${schedules[i].snum}</i>
                            <div class='text-decoration-none text-black w-75'><strong class='mb-1 place-title plans-title' data-splace="${schedules[i].spoint}">${schedules[i].spoint}</strong></div>
                            <small><button class='btn-outline-success rounded-3 border-1' onclick="planDelete(${schedules[i].snum})">삭제</button></small>
                        </div>
                        <input class='col-10 w-100 m-0 mt-2 small plans-memo' style='resize:none;' value='${schedules[i].smemo}' />
                    </div>
                </c:if>
            </c:forEach>
        </div>
...
<script>
    ...
    function getListItem(index, places) {
        ...
        var itemStr = '<div class="d-flex w-100">' + '<div class="markerbg marker_' + (index+1) + '">'+(index+1)+'</div>' +
    ...
    function submitTest() {
        ...
        if(nullCheck == true) {
            ...
            $.ajax({
                ...
                type:"post",
                success: function () {
                    location.href = "${path}/planner/${planNum}"
                }
            });
        }
    }
