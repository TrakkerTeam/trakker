    ...
    <div class="d-flex row edit_view">
        ...
        <div class="col-sm-11 ps-5">
            ...
            <div class="col-sm-11">
                ...
                <p class="mb-2 opacity-70">
                    <c:choose>
                        <c:when test="${row.comment_p > 1}">
                            <c:if test="${row.com_delete == '0'}">
                                <strong class="pe-2"
                                        style="color: #A3A3A3">${row.parent_comment_nickname}</strong> ${str}
                            </c:if>
                ...
                <c:if test="${row.com_delete == '0'}">
                    <small class="text-muted text-nowrap"><fmt:formatDate value="${row.com_date}"
            ...
            <c:if test="${row.com_delete == '0'}">
                <div class="col-sm-1 align-self-end">
                    ...
                    <c:if test="${sessionScope.mem_nickname == row.member.mem_nickname}">
                        <button class="btn btn-outline-success editDiv" type="button"
                                data-comment_num="${row.comment_num}" data-review_num="${row.review_num}"
                                data-mem_num="${row.mem_num}" data-lnum="${row.lnum}" data-content="${row.content}">수정
                        </button>
                        <button class="btn btn-outline-success mt-3" type="button"
                                onclick="commentDelete(${row.comment_num})">삭제
                        </button>
                    </c:if>
                </div>
            </c:if>
